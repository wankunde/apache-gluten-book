...
// 使用示例
int fd = createSharedMemory("/gluten-shuffle", 1024 * 1024 * 1024); // 1GB
void* addr = mapSharedMemory(fd, 1024 * 1024 * 1024);

// 现在多个进程可以访问这块内存
```

### 7.4.2 Arrow Flight（未来可能采用）

**Arrow Flight**：基于 gRPC 的高性能数据传输协议
...
class MyFlightServer : public arrow::flight::FlightServerBase {
  arrow::Status DoGet(
    ...
    // 返回 RecordBatch 流
    auto batches = loadDataFromStorage();
    *stream = std::make_unique<arrow::flight::RecordBatchStream>(
      arrow::RecordBatchReader::Make(batches)
    );
...
auto stream = client.DoGet(options, ticket);
...
### 7.4.3 RDMA（Remote Direct Memory Access）

**在高性能网络中使用**：

```cpp
// RDMA 零拷贝传输
struct ibv_mr* registerMemory(void* addr, size_t size) {
  return ibv_reg_mr(pd, addr, size, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
}

void rdmaWrite(struct ibv_qp* qp, struct ibv_mr* mr, uint64_t remote_addr) {
